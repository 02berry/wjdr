...
- **坐标模型：** 玩家坐标 (gx,gy) 指向所占 2×2 块的**底部格子**，菱形顶点 `to_display(gx,gy)→(gx+2,gy)→(gx+2,gy+2)→(gx,gy+2)`
- **地图几何中心：** `MAP_CX, MAP_CY = 599, 599`（用于地形分区，确保荒原环与地图边界对齐）
- **地图边框：** 从 `(-1,-1)` 到 `(1200,1200)`，包住 0~1199 的格子范围
- 聚落检测：迭代剔除离群点（半径≤30，最少20人），360°扫描找零遮挡标签位置
- 矿工高亮：双色竖切（左半原色 + 右半黑色）
- 地形分区：荒原(1200)、雪原(500)、沃土(200) 菱形环，以地图几何中心 (599,599) 对齐
